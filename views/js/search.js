...
searchbutton.addEventListener('click', async ()=>{
    ...
    console.log(searchbar)
    if(searchbar!=""){
        await axios.get(`https://saavnapi-nine.vercel.app/result/?query=${searchbar}`)
        .then((res)=>{
            ...
            for (let j = 0; j < searchcontent.length; j++) {
                ...
                searchresult.innerHTML += `<li>
                    <div class="image_play">
                        <img src="${searchcontent[j].image}" alt="">
                        <img class="search_play_icon" id="${searchcontent[j].perma_url}" src="../images/play.svg" alt="">
                    </div>
                    <div>
                    <h5 style="font-size:15px" >${searchcontent[j].song}</h5>
                    <h5>${searchcontent[j].primary_artists}</h5>
                    </div>
            </li>`
            }
            searchSongArray = Array.from(document.getElementsByClassName('search_play_icon'));
